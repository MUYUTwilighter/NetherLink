...
class NliLinkServiceContractTest {
    ...
    void registersRuntimeUsesInstanceTokenAndParsesMultiPresenceFriends() throws Exception {
        ...
        try {
            ...
            LinkContextHooks.setClientConnection(runtimeKey, new TestAccount(), "Test client", ignored -> {});
            ...
            assertEquals(2, snapshot.friends().get(0).presences().size());
            assertEquals(LinkPresenceStatus.HOSTING, snapshot.friends().get(0).presences().get(0).status());
            assertEquals(LinkFriendRelationship.INCOMING, snapshot.incoming().get(0).relationship());
    ...
    void refreshesMinecraftTokenAndRetriesRuntimeRegistrationAfterUnauthorized() throws Exception {
        ...
        NliLinkService service = new NliLinkService(java.net.URI.create("http://127.0.0.1:" + server.getAddress().getPort()));
        try {
            LinkContextHooks.setServerConnection(runtimeKey, account, "Test server", (ignoredA, ignoredB) -> {}, () -> {
                refreshed.set(true);
