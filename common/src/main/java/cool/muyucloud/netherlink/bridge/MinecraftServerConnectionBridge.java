    ...
    private void acceptOnServerThread(Channel channel) {
        ...
        channel.pipeline().addLast(new ChannelInitializer<>() {
            ...
            protected void initChannel(Channel ch) {
                ...
                connection.setListener(new ServerHandshakePacketListenerImpl(MinecraftServerConnectionBridge.this.server, connection));
                listener.getConnections().add(connection);
            }
        });
        ServerConnectionListener.SERVER_EVENT_GROUP.get().register(channel).syncUninterruptibly();
    }
}
