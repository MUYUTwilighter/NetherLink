...
public final class JsonRpcClient implements WebSocket.Listener {
    ...
    public CompletableFuture<JsonElement> sendRequest(String method, List<JsonElement> params) {
        ...
        this.executor.execute(() -> {
            ...
            this.sendChain = this.sendChain.<Void>thenCompose(ignored1 -> ws.sendText(payload, true).thenApply(ignored2 -> null)).exceptionally(error -> {
                this.executor.execute(() -> {
    ...
    public CompletableFuture<?> close() {
        ...
        this.executor.execute(() -> {
            ...
            if (ws != null && !ws.isOutputClosed()) {
                ws.sendClose(1000, "shutdown").whenComplete((ignored1, ignored2) -> done.complete(null));
            } else {
                done.complete(null);
            }
        });
