...
final class NliSignalingClient implements LinkSignalingClient, WebSocket.Listener {
    ...
    public CompletableFuture<Void> sendClientMessage(LinkPeerRoute target, SignalingMessage message) {
        ...
        envelope.add("payload", Util.getOrThrow(
            SignalingMessage.CODEC.encodeStart(JsonOps.INSTANCE, message),
            error -> new IllegalStateException("Cannot encode signaling message: " + error)
        ));
    ...
    private void handleFrame(String frame) {
        try {
            ...
            SignalingMessage message = Util.getOrThrow(
                SignalingMessage.CODEC.parse(JsonOps.INSTANCE, payload),
                error -> new IllegalStateException("Malformed NLI signaling payload: " + error)
            );
