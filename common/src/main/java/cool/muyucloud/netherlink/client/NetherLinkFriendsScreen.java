...
public class NetherLinkFriendsScreen extends Screen {
    ...
    private final class SettingsTab implements Tab {
        ...
        private SettingsTab() {
            ...
            this.receiveRequestsButton = CycleButton.onOffBuilder(false)
                .create(0, 0, 180, 20, Component.translatable("netherlink.friends.settings.requests"), (ignored2, ignored102) -> this.updateButtons());
            this.updateButtons();
            this.loadSettings();
        }
        ...
        private void apply() {
            ...
            ClientLinkSettings.use(NetherLinkFriendsScreen.this.minecraft, this.selectedService)
                .whenComplete((ignored3, error) -> NetherLinkFriendsScreen.this.minecraft.execute(() -> {
                    if (error != null) {
    ...
    private static final class FriendRow extends Row {
        ...
        private FriendRow(ClientFriendService.Friend friend) {
            this.friend = friend;
            this.skin = Minecraft.getInstance().getSkinManager().lookupInsecure(new GameProfile(friend.profileId(), friend.name()));
        }
    ...
    private static final class RequestRow extends Row {
        ...
        private RequestRow(ClientFriendService.Request request) {
            ...
            this.skin = Minecraft.getInstance().getSkinManager().lookupInsecure(new GameProfile(request.profileId(), request.name()));
        }
