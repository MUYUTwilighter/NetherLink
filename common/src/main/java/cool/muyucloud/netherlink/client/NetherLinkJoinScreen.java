...
final class NetherLinkJoinScreen extends Screen {
    ...
    @Override
    protected void init() {
        this.cancelButton = this.addRenderableWidget(Button.builder(CommonComponents.GUI_CANCEL, _ -> this.cancelOrClose())
    ...
    private void start() {
        ...
        this.operationFuture.whenComplete((result, error) -> this.minecraft.execute(() -> {
            ...
            this.operation.completion().whenComplete((_, completionError) -> this.minecraft.execute(() -> {
                if (this.terminal) {
                    return;
                }
    ...
    public void extractRenderState(@NonNull GuiGraphicsExtractor graphics, int mouseX, int mouseY, float partialTick) {
        ...
        graphics.centeredText(this.font, this.title, this.width / 2, this.height / 2 - 58, -1);
        graphics.centeredText(this.font, this.status, this.width / 2, this.height / 2 - 30, -1);
        if (this.detail != null) {
            graphics.centeredText(this.font, this.detail, this.width / 2, this.height / 2 - 12, -1);
        }
    }
}
